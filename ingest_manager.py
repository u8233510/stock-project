# --- ingest_manager.py (精簡版：移除主動流向運算) ---
import time
import sqlite3
import pandas as pd
...
def process_data(key, df, table, conn):
    if df is None or df.empty: return pd.DataFrame()
    db_cols = database.get_table_columns(conn, table)
    clean_to_real_db = {c.lower(): c for c in db_cols}
    final_rename = {df_c: clean_to_real_db[df_c.lower()] for df_c in df.columns if df_c.lower() in clean_to_real_db}
...
def get_missing_ranges(conn, table_name, stock_id, date_col, target_start):
    ...
    try:
        sql = f"SELECT MIN({date_col}), MAX({date_col}) FROM {table_name} WHERE stock_id = ? AND {date_col} != ''"
        res = conn.execute(sql, (stock_id,)).fetchone()
        db_min_str, db_max_str = (str(res[0]) if res[0] else None), (str(res[1]) if res[1] else None)
    except: return [(t_start, t_end)]
    ranges = []
    if not db_min_str: ranges.append((t_start, t_end))
    else:
        db_min = datetime.strptime(db_min_str[:10], "%Y-%m-%d").date()
        db_max = datetime.strptime(db_max_str[:10], "%Y-%m-%d").date()
        if t_start < db_min: ranges.append((t_start, db_min - timedelta(days=1)))
        if t_end > db_max: ranges.append((db_max + timedelta(days=1), t_end))
    return ranges

def start_ingest(st_placeholder=None):
    failed_log = []
    cfg = database.load_config()
    api = DataLoader(); api.login_by_token(api_token=cfg["finmind"]["api_token"])
    ...
    def log(msg):
        if st_placeholder: st_placeholder.write(msg)
        print(msg)
                    ...
                    if key == "branch":
                        for d_str in pd.date_range(start, end).strftime("%Y-%m-%d"):
                            ...
                            if df is not None and not df.empty:
                                upsert_data(conn, table, process_data(key, df, table, conn))
