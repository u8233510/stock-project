...
def fetch_securities_trader_info(api: DataLoader) -> pd.DataFrame:
    return api.taiwan_securities_trader_info()


def upsert_securities_trader_info(conn: sqlite3.Connection, df: pd.DataFrame):
    if df is None or df.empty:
        return 0

    out = df.copy()
    needed = ["date", "securities_trader_id", "securities_trader", "address", "phone"]
    for col in needed:
        if col not in out.columns:
            out[col] = None
    ...
    out = out.dropna(subset=["date", "securities_trader_id"]).drop_duplicates()

    payload = [tuple(row) for row in out.itertuples(index=False, name=None)]
    conn.executemany(
        """
        INSERT OR REPLACE INTO finmind_securities_trader_info(
            date, securities_trader_id, securities_trader, address, phone
        ) VALUES (?, ?, ?, ?, ?)
        """,
        payload,
    )
...
def ensure_branch_tables(conn: sqlite3.Connection, write_legacy_branch_table: bool):
    database.ensure_finmind_branch_trade_detail_schema(conn)
    database.ensure_finmind_securities_trader_info_schema(conn)
    database.ensure_branch_sync_log_schema(conn)
    if write_legacy_branch_table:
        conn.execute(database.TABLE_REGISTRY["branch"])
        conn.commit()


def upsert_finmind_branch_detail_df(conn: sqlite3.Connection, df: pd.DataFrame):
    if df.empty:
        return
    payload = [
        (
    ...
    conn.executemany(
        """
        INSERT OR REPLACE INTO finmind_branch_trade_detail(
            date, securities_trader_id, securities_trader, stock_id, price, buy, sell
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        payload,
    )
...
def upsert_branch_price_legacy_df(conn: sqlite3.Connection, df: pd.DataFrame):
    if df.empty:
        return
    payload = [
        (
...
def run_collection(
    ...
    write_legacy_branch_table: bool = True,
    sync_trader_info: bool = True,
) -> pd.DataFrame:
    api = DataLoader()
    ...
    try:
        if conn is not None:
            ensure_branch_tables(conn, write_legacy_branch_table=write_legacy_branch_table)

        if conn is not None and sync_trader_info:
            trader_df = fetch_securities_trader_info(api)
            trader_rows = upsert_securities_trader_info(conn, trader_df)
            if progress_callback:
                progress_callback(f"已同步券商主檔: {trader_rows} 筆")
        ...
        for trade_date in daterange(start_date, end_date):
            for branch_id in branch_ids:
                ...
                try:
                    ...
                    if conn is not None:
                        upsert_finmind_branch_detail_df(conn, clean_df)
                        if write_legacy_branch_table:
                            upsert_branch_price_legacy_df(conn, clean_df)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-legacy-branch-price",
        action="store_true",
        help="If set, do not write branch_price_daily (only finmind_branch_trade_detail).",
    )
    parser.add_argument(
        "--skip-trader-info-sync",
        action="store_true",
        help="If set, do not sync TaiwanSecuritiesTraderInfo before detail ingestion.",
    )
...
def main():
    ...
    run_collection(
        ...
        write_legacy_branch_table=not bool(args.skip_legacy_branch_price),
        sync_trader_info=not bool(args.skip_trader_info_sync),
    )
