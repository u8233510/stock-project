...
def _load_scan_raw(conn, start_date: str, end_date: str, stock_ids: list[str]) -> pd.DataFrame:
    ...
    WHERE b.date BETWEEN ? AND ?
      AND b.stock_id IN ({placeholders})
    ORDER BY b.date ASC
    """
...
def show_branch_accumulation_scan():
    ...
    st.caption("先掃分點再找股票：找出哪個分點在多檔股票出現『連續買超、只買不賣、吃下成交量』。")
    ...
    default_stock_ids = [s["stock_id"] for s in universe]
    ...
    if not min_date or not max_date:
        st.info("目前資料庫沒有可掃描的分點資料。")
        return

    c1, c2 = st.columns([2.8, 1.2])
    with c1:
        selected_stock_labels = st.multiselect(
            "掃描股票池（可複選，不選則預設全 universe）",
            options=list(stock_options.keys()),
            default=[],
            key="acc_scan_stock_pool",
        )
    with c2:
    ...
    selected_stock_ids = [stock_options[label] for label in selected_stock_labels] if selected_stock_labels else default_stock_ids

    state_key = "accumulation_scan_result"
    if run:
        raw_df = _load_scan_raw(conn, str(start_d), str(end_d), selected_stock_ids)
        if raw_df.empty:
            st.info("選定區間與股票池內無可用資料。")
    ...
    c1.metric("符合條件分點×股票", f"{len(result_df)}")
