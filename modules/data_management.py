...
def show_data_management():
    ...
    if task_type == "📅 每日 13 項指標 (原 Ingest Manager)":
        ...
        default_write_raw_files = bool(branch_sync_cfg.get("write_raw_files", False))
        ...
        with col2:
            ...
            write_raw_files = st.checkbox(
                "保留 Raw CSV（較慢）",
                value=default_write_raw_files,
                help="關閉可避免每筆分點明細另存 CSV，通常會更快；僅在除錯或稽核時建議開啟。",
            )
        ...
        if st.button("🚀 啟動分點明細同步", use_container_width=True):
            ...
            with st.spinner("分點明細同步中..."):
                stats = run_collection(
                    ...
                    write_raw_files=write_raw_files,
                    max_retries=int(max_retries),
                    retry_sleep_sec=float(retry_sleep_sec),
